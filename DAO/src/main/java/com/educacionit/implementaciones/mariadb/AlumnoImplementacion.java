...
public class AlumnoImplementacion implements DAO<Alumno, Documento>, ConexionMariaDB {
	...
	private boolean insertar(Alumno alumno) {
		...
		try {
			...
			psInsertar.setDate(4, Date.valueOf(Fechas.getFechaAStringSQL(alumno.getFechaNacimiento())));
	...
	private boolean modificar(Alumno alumno) {
		...
		String query = "update alumnos set descripcion = ?, fechaNacimiento = ?, activo = ? where numeroDocumento = ? and tipoDocumento = ?";
		...
		try {
			...
			psModificar.setDate(2, Date.valueOf(Fechas.getFechaAStringSQL(alumno.getFechaNacimiento())));
